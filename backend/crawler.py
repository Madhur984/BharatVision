...
class EcommerceCrawler:
    ...
    def _enrich_product(self, product: ProductData, platform: str):
        ...
        try:
            if self.compliance_validator:
                ...
                product.compliance_score = max(0, 100 - (violations * (100 / total_rules)))
    ...
    def _run_llm_extract(self, text: str) -> Dict[str, Any]:
        ...
        try:
            # Lazy load model
            if getattr(self, '_nlp_model', None) is None:
                try:
                    ...
                    try:
                        ...
                        logger.info("Loading NLP model with 4-bit quantization...")
                    except (importlib.metadata.PackageNotFoundError, ImportError):
                        logger.info("Loading NLP model without quantization (bitsandbytes not available)...")
                    ...
                    if use_quantization:
                        ...
                        self._nlp_model = AutoModelForCausalLM.from_pretrained(
                            model_id,
                    ...
                    logger.info(f"NLP model loaded successfully: {model_id}")
                except Exception as e:
            ...
            truncated = (text or '')
            truncated = re.sub(r"\s+", " ", truncated).strip()[:1500] # Increased context window for Gemma
...
""" }
            ...
            inputs = self._tokenizer(prompt, return_tensors="pt").to(self._nlp_model.device)

            outputs = self._nlp_model.generate(
                **inputs,
                max_new_tokens=256,
                do_sample=False,
                temperature=0.0
            ...
            gen_text = self._tokenizer.decode(outputs[0], skip_special_tokens=True)
            # Remove the prompt part if it's included (Gemma usually just appends, but safe to slice if needed)
            # We'll just look for the JSON block in the full output
            
            try:
                import json as _json
                # Find first { and last }
                m = re.search(r'\{(?:[^{}]|(?R))*\}', gen_text, flags=re.S) # Recursive regex not supported slightly, use simpler inner match
                # Simple greedy match for JSON object
                start = gen_text.find('{')
                end = gen_text.rfind('}')
                if start != -1 and end != -1:
                    json_str = gen_text[start:end+1]
                    parsed = _json.loads(json_str)
                    return parsed if isinstance(parsed, dict) else regex_extract(text)
            except Exception:
                pass
            ...
            logger.exception("NLP extraction failed, falling back to regex")
            return regex_extract(text)
    ...
    def extract_product_from_url(self, product_url: str) -> Optional[ProductData]:
        ...
        try:
            ...
            product = None
            if platform == 'amazon':
                product = self._extract_amazon_details(soup, product_url)
            elif platform == 'flipkart':
